...
for update or insert on asistente compound trigger

newAsistente asistente%rowtype;
...
newAsistente.DNI:= :new.DNI;
newAsistente.NOMBRE:= :new.NOMBRE;
newAsistente.TELEFONO:= :new.TELEFONO;
newAsistente.DIRECCION:= :new.DIRECCION;
newAsistente.ID_EQUIPO:= :new.ID_EQUIPO;
newAsistente.SUELDO:= :new.SUELDO;
end if;
end before each row;
...
select count(*) into v_numAsistentes
from asistente
where id_equipo= v_cod_equipo.COD_EQUIPO;
...
raise_application_error(-20011,'ERROR: EQUIPO '||v_cod_equipo.COD_EQUIPO||' YA TIENE UN ENTRENADOR');
END IF;
end loop;
